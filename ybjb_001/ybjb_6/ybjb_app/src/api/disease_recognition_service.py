...
from ybjb_app.model.maxvit import MaxViTTwoInput
...
class DiseasePredictor:
    ...
    def batch_predict(self, patient_list, output_dir="D:\\defect_detection\\ybjb_app_2\\ybjb_app\\reports"):
        """批量预测"""
    ...
    def _visualize_prediction(self, left_img, right_img, df, save_path):
        ...
        for i, (img, title) in enumerate(zip([left_img, right_img], ["左眼", "右眼"])):
            ax = fig.add_subplot(1, 2, i + 1)
            ax.imshow(img)
            ax.set_title(f"{title} 图像", fontsize=12)
            ax.axis('off')
...
if __name__ == "__main__":
    # 初始化预测器
    predictor = DiseasePredictor("./snapshot/kappa=0.6.pth")

    # 准备测试数据（需准备实际图像文件）
    test_patients = [
        ("D:\defect_detection\dates\微信图片_20250317194411.jpg", "D:\defect_detection\dates\微信图片_20250317194507.jpg"),  # 真实双眼
        ("D:\defect_detection\dates\微信图片_20250317194510.jpg", None),
    ]
